...
\label{actual-project-timeline}
 ...
 \input{ProjectManagementResults/ProjectTimeline/TaskSummary.tex}
 \input{ProjectManagementResults/ProjectTimeline/ActualGanttDiagram.tex}
...
During project execution, dedicating one week to \textbf{Dev3} task (CI/CD Pipeline Implementation) revealed a shortfall compared to the original approximation of five days due to the author's lack of expertise. The execution was already exceeding the planned allocation, the task is not high-priority and has no strong dependencies on other tasks. \textbf{Dev3} was rescheduled to the final development stage and ultimately discarded. By the project's final stage, no buffer time remained for its completion, leaving the possibility to implement this task in future iterations.
