\subsubsection{Main Use Cases Sequence Diagrams}
...
The \texttt{ExpenseService} first calls the \texttt{BlobStorageService} to upload the expense receipt file to Azure Blob Storage. After the upload is completed, the \texttt{BlobStorageService} returns the generated blob URL. Using this URL and the request information, the \texttt{Expense- Service} creates a new \texttt{Expense} entity and passes it to the \texttt{ExpenseRepository}. The repository is responsible for persisting the entity by interacting with the \texttt{ApplicationDb- Context} through Entity Framework Core (EF Core), which ultimately stores the expense metadata in the SQL database.
...
This second use case demonstrates how the asynchronous email notification is triggered to provide real-time feedback when a report status changes.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Images/SeqDiagrams/seq4.png}
...
The \texttt{ReportService} first verifies that the report exists and that the assigned reviewer matches the authenticated user. If either validation fails, the service returns null to the \texttt{ReportsController}, which subsequently returns a 404 Not Found response to the client (UI).
