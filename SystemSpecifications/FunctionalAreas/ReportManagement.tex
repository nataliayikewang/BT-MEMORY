\subsubsection{Report Management}
\label{sec:report-management}

This functional area groups all the operations related to report management. In this application, a report serves as a container for a set of expenses prepared for submission. Once a report has been reviewed and finalized, it is submitted to the appropriate reviewer. Core functionalities include creating, modifying, deleting, and submitting reports. Additionally, expenses can be easily assigned to a report, and any assigned expenses can be updated while preserving data consistency. The use case diagram for this functional area is shown in Figure \ref{fig:report-management-use-case-diagram}.


\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Images/reportmanagement.png}
    \caption[Report management use case diagram]
    {Report management use case diagram. [Own creation]}
    \label{fig:report-management-use-case-diagram}
\end{figure}