...
A Gantt diagram is attached at the end of this section. Notice that the execution order aligns with the prioritized requirements defined in Section \ref{sec:functional-requirements} based on MoSCoW criteria. Continuous arrows represent Finish-Start (FS) dependencies between tasks, while dashed arrows represent Finish-Finish (FF) dependencies. FS dependencies indicate that successor tasks cannot start until their predecessor tasks are completed. FF dependencies show that successor tasks cannot finish until their predecessor tasks are completed.
...
\begin{itemize}[leftmargin=0.75cm, noitemsep, label=\textbullet]  
    ...
    \item \textbf{PM3} starts after \textbf{PS2} (temporal planning) is completed and overlaps with full application development, as the product backlog requires regular updates based on the project's real-time progress. 
    \item \textbf{FPD2} runs throughout the project execution for final delivery consistency.
...
\end{itemize}
In addition, \textbf{ER1}, \textbf{ER3}, \textbf{UA2}, and \textbf{UA6} tasks have an initial effort estimation of 8 hours. However, the additional time allocated serves as a buffer for likely challenges. \textbf{ER1} is the first coding task; \textbf{ER3} requires extensive coordination with frontend and backend; \textbf{UA6} marks the author's first experience implementing email notifications; \textbf{UA2} requires a prior database record of the employee hierarchy. Finally, \textbf{Backend Development} tasks similarly incur additional time costs.
...
\begin{figure}[H]
    ...
    \caption[Initial project Gantt diagram]
    {Initial project Gantt diagram. [Own creation]} 
