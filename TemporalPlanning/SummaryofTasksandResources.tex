\subsection{Summary of Tasks and Resources}

See Table \ref{table:summary-of-tasks-with-effort-dependencies-and-resources} for a summary of all defined tasks, including their identifiers, estimated effort in hours, any final-to-start dependencies with other tasks, and the required resources. The final-to-start dependency column indicates which task must be fully completed before the task in that row can begin.

%Table of personnel cost breakdown by task and role
\begin{landscape}
\pagestyle{fancy}
\scriptsize
\setlength{\tabcolsep}{3pt}
\setlength{\arrayrulewidth}{0.1pt}
\renewcommand{\arraystretch}{1.15}
\begin{longtable}{l l r l l}
\rowcolor{gray!45}
\textbf{ID} & \textbf{Name} & \textbf{Time (h)} & \textbf{Dependencies} & \textbf{Resources} \\
\addlinespace[0.2em]
\endfirsthead

\multicolumn{5}{l}{\textit{(Continued from previous page)}} \\
\rowcolor{gray!45}
\textbf{ID} & \textbf{Name} & \textbf{Time (h)} & \textbf{Dependencies} & \textbf{Resources} \\
\addlinespace[0.2em]
\endhead

\hline 
\multicolumn{5}{r}{\textit{(Continued on next page)}} \\
\endfoot

\hline 
\addlinespace[0.2em]
\rowcolor{gray!45}
\textbf{Total} & & \textbf{513} & & \\
\caption[Summary of project tasks with effort, dependencies, and resources]
{Summary of project tasks with effort, dependencies, and resources. [Own creation]} 
\vspace{-1em}
\label{table:summary-of-tasks-with-effort-dependencies-and-resources} 
\endlastfoot

\rowcolor{gray!20}
\textbf{PS} & \textbf{Project Starting} & \textbf{50} & & \\ 
\rowcolor{gray!05}
PS1 & Context and Scope & 20 & - & SM, PO, TD, GT, DT, OS, HW, MCS, VSC, FIB, GM, WEB, OVL \\
\rowcolor{gray!05}
PS2 & Temporal Planning & 10 & PS1 & SM, PO, TD, GT, DT, OS, HW, MCS, VSC, FIB, GM, WEB, OVL, OT \\ 
\rowcolor{gray!05}
PS3 & Budget and Sustainability & 10 & PS2 & SM, PO, TD, GT, DT, OS, HW, MCS, VSC, FIB, GM, WEB, OVL \\ 
\rowcolor{gray!05}
PS4 & Initial Documentation & 10 & PS3 & SM, PO, TD, GT, DT, OS, HW, MCS, VSC, FIB, GM, WEB, OVL \\ 
 
\rowcolor{gray!20}
\textbf{PM} & \textbf{Project Management} & \textbf{67} & & \\ 
\rowcolor{gray!05}
PM1 & Agile Ceremonies & 51 & PS2 & SM, PO, DT, OS, HW, AZR, MCS, VSC \\ 
\rowcolor{gray!05}
PM2 & Thesis Director Review Meetings & 8 & - & DT, TD, OS, HW, AZR, VSC, FIB, GM, WEB \\ 
\rowcolor{gray!05}
PM3 & Product Backlog Update & 8 & PS2 & SM, DT, PO, OS, HW, AZR, WEB \\ 

\rowcolor{gray!20}
\textbf{FD} & \textbf{Final Product Delivery} & \textbf{46} & & \\ 
\rowcolor{gray!05}
FPD1 & User Manual Documentation & 2 & PM5 & DT, OS, HW, VSC, WEB \\ 
\rowcolor{gray!05}
FPD2 & Final Thesis Documentation & 20 & - & DT, TD, OS, HW, FIB, GM, WEB, OVL, OT \\ 
\rowcolor{gray!05}
FPD3 & Final Demonstration Video & 4 & FPD2 & DT, OS, HW, WEB \\ 
\rowcolor{gray!05}
FPD4 & Bachelor's Thesis Defense & 20 & FPD2 & DT, TD, OS, HW, FIB, GM, WEB \\ 

\rowcolor{gray!20}
\textbf{DT} & \textbf{Design} & \textbf{16} & & \\ 
DT1 & Use Case Diagram & 2 & PS1 & DT, OS, HW, OT, WEB, MCS \\ 
\rowcolor{gray!05}
DT2 & Conceptual Model Diagram & 4 & PS1 & DT, OS, HW, OT, WEB \\ 
\rowcolor{gray!05}
DT3 & Technology Stack Decision & 2 & PS1 & DT, OS, HW, WEB, MCS \\ 
\rowcolor{gray!05}
DT4 & Logical and Physical Architecture Design & 4 & PS1 & DT, OS, HW, WEB, MCS \\ 
\rowcolor{gray!05}
DT5 & UI mockups Design & 2 & DT1 & DT, OS, HW, OT, WEB \\ 
\rowcolor{gray!05}
DT6 & Security Design & 2 & DT3, DT4 & DT, OS, HW, WEB, MCS \\ 

\rowcolor{gray!20}
\textbf{Dev} & \textbf{Development} & \textbf{76} & & \\ 
\rowcolor{gray!05}
Dev0 & Self-learning and study of unfamiliar technologies & 8 & DT3 & DT, OS, HW, WEB \\ 
\rowcolor{gray!05}
Dev1 & Environment Configuration & 2 & DT2, DT5, DT6 & DT, OS, HW, GIT, AZR, VSC, WEB \\ 
\rowcolor{gray!05}
Dev2 & Product Deployment & 8 & Dev3 & DT, OS, HW, GIT, AZR, VSC, WEB \\ 
\rowcolor{gray!05}
Dev3 & Unit and Integration Testing & 10 & UA4, UA5, UA6 & DT, OS, HW, GIT, AZR, VSC, WEB \\ 
\rowcolor{gray!05}
Dev4 & Logging and Monitoring & 8 & Dev2 & DT, OS, HW, GIT, AZR, VSC, WEB \\ 
\rowcolor{gray!05}
Dev5 & Git Repository Documentation & 40 & Dev1 & DT, OS, HW, GIT, AZR, VSC, WEB \\ 

\rowcolor{gray!20}
\multicolumn{2}{l}{\textbf{Frontend and Backend Development}} & \textbf{128} & & \\ 

\rowcolor{gray!10}
\textbf{ER} & \textbf{Expense and Report Development} & \textbf{64} & & \\ 
\rowcolor{gray!05}
ER1 & Expense Uploads & 16 & Dev1 & DT, OS, HW, GIT, AZR, VSC, WEB \\ 
\rowcolor{gray!05}
ER2 & Uploaded Expenses Real-time Dashboard & 4 & ER1, BD6 & DT, OS, HW, GIT, AZR, VSC, WEB \\ 
\rowcolor{gray!05}
ER3 & AI-Extracted Expense Data Review and Correction & 20 & ER2 & DT, OS, HW, GIT, AZR, VSC, WEB \\ 
\rowcolor{gray!05}
ER4 & Uploaded Expenses Deletion & 4 & ER2 & DT, OS, HW, GIT, AZR, VSC, WEB \\ 
\rowcolor{gray!05}
ER5 & Expense Reports View, Creation, Edit, Submission, and Deletion & 20 & ER3, ER4 & DT, OS, HW, GIT, AZR, VSC, WEB \\

\rowcolor{gray!10}
\textbf{UA} & \textbf{User Access Development} & \textbf{64} & & \\  
\rowcolor{gray!05}
UA1 & Company Email User Authentication & 8 & BD7 & DT, OS, HW, GIT, AZR, VSC, WEB \\
\rowcolor{gray!05}  
UA2 & Role-based User Access & 16 & UA1 & DT, OS, HW, GIT, AZR, VSC, WEB \\ 
\rowcolor{gray!05}
UA3 & Submitted Report Approval and Rejection & 8 & ER5, UA2 & DT, OS, HW, GIT, AZR, VSC, WEB \\ 
\rowcolor{gray!05}
UA4 & Employee Spending Limit Configuration & 8 & UA2 & DT, OS, HW, GIT, AZR, VSC, WEB \\ 
\rowcolor{gray!05}
UA5 & Submitted and Reviewed Expenses Record Visualization & 4 & UA3 & DT, OS, HW, GIT, AZR, VSC, WEB \\ 
\rowcolor{gray!05}
UA6 & Email Notification Service Integration & 20 & UA3 & DT, OS, HW, GIT, AZR, VSC, WEB \\ 

\rowcolor{gray!20}
\textbf{BD} & \textbf{Backend Development} & \textbf{120} & & \\
\rowcolor{gray!05}
BD1 & RESTful API Backend Logic Implementation & 20 & Dev1 & DT, OS, HW, GIT, AZR, VSC, WEB \\ 
\rowcolor{gray!05}
BD2 & Azure SQL Database Integration and Manipulation & 20 & BD1 & DT, OS, HW, GIT, AZR, VSC, WEB \\ 
\rowcolor{gray!05}
BD3 & Azure Blob Storage Integration & 8 & BD2 & DT, OS, HW, GIT, AZR, VSC, WEB \\ 
\rowcolor{gray!05}
BD4 & Microsoft Document Intelligence AI Model Integration & 20 & BD2, BD3 & DT, OS, HW, GIT, AZR, VSC, WEB \\
\rowcolor{gray!05}
BD5 & Automated AI-driven Data Extraction Pipeline & 20 & BD4 & DT, OS, HW, GIT, AZR, VSC, WEB \\ 
\rowcolor{gray!05}
BD6 & Azure SignalR Real-time Notification Service Integration & 16 & BD5 & DT, OS, HW, GIT, AZR, VSC, WEB \\ 
\rowcolor{gray!05}
BD7 & Microsoft Entra ID User Authentication Integration & 16 & BD1 & DT, OS, HW, GIT, AZR, VSC, WEB \\ 

\rowcolor{gray!20}
\textbf{FD} & \textbf{Frontend Development} & \textbf{10} & & \\ 
\rowcolor{gray!05}
FD1 & Expenses Search and Filter & 8 & ER2 & DT, OS, HW, GIT, AZR, VSC, WEB \\ 
\rowcolor{gray!05}
FD2 & Reports Search & 2 & ER5 & DT, OS, HW, GIT, AZR, VSC, WEB \\ 

\end{longtable}
\end{landscape}


