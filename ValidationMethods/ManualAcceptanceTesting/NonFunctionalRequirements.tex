...
\vspace{20pt}
    ...
    {Summary of non-functional requirements and their acceptance criteria. [Own creation]} 
    ...
    \rowcolor{gray!05}
    NFR2 & The UI must be {intuitive}, allowing new users to learn how to use the application within \textbf{3 minutes}.\\
    \rowcolor{gray!05}
    ...
    \rowcolor{gray!05}
    NFR7 & The application's code implementation must use \textbf{interfaces} to interact with external services, place the concrete implementations in a separate layer, and adopt the \textbf{Dependency Inversion Principle}. \\
    \rowcolor{gray!05}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{
        ...
        \rowcolor{gray!05}
        NFR3 &
        Loading uploaded expenses in a new session &
        3 s &
        0.128 s \\

        \rowcolor{gray!05}
        NFR3 &
        Performing basic user interactions &
        500 ms &
        32 ms \\
    \end{tabularx}
...
Although these scenarios can be interpreted positively in terms of user adoption, they introduced variability in the measurements and were therefore excluded from the final results.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{
        ...
        \rowcolor{gray!05}
        NFR1 &
        Updating expense processing status in real time &
        1 s &
        0. 478 s \\

        \rowcolor{gray!05}
        NFR2 &
        Learning the application usage curve &
        3 min &
        2 min 24 s \\

        \rowcolor{gray!05}
        NFR6 &
        Completing the full expense workflow &
        5 min &
        2 min 42 s \\
    \end{tabularx}
...
The remaining NFRs are justified in Table \ref{table:other-nfr-justification}.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{
        ...
        \rowcolor{gray!05}
        NFR2 &
        During manual acceptance testing for this requirement, most users relied on the user manual for guidance. After uploading a receipt, the process proved ambiguous, as users were not aware that only manually reviewed AI-extracted expenses with the status ``Reviewed'' could be added to a report. In addition, most users did not know how to submit uploaded expenses for approval because the application requires them to create a report grouping those expenses before it can be sent to a reviewer.

        To address this issue, a detailed user guide pop-up was added to the user interface. This pop-up provides instructions to guide the user; the corresponding screenshot is shown in Appendix \ref{appendix:user_interface_gallery}, Figure \ref{fig:user-guide-popup-appendix}. As no further ambiguities were identified, this adjustment rendered the interface more intuitive for users.\\

        \rowcolor{gray!05}
        NFR3 &
        The application's resources are deployed through the Azure platform, therefore, they are cloud-based and are available 24 hours a day, 365 days a year. The deployment details are provided in the next section. \\

        \rowcolor{gray!05}
        NFR4 &
        The application's hybrid architecture is detailed in Section \ref{sec:physical-architecture} and fulfills the defined acceptance criteria. Moreover, by deploying the application's resources on the Azure platform, these resources support automatic horizontal scaling. \\
        ...
        \rowcolor{gray!05}
        NFR7 &
        The Dependency Inversion Principle and the “Programming to an Interface” pattern are applied throughout the application implementation and are stated in Section \ref{sec:principles-and-patterns}. \\
