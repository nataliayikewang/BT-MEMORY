...
\begin{itemize} [leftmargin=0.75cm, itemsep=0.5pt, label=\textbullet]
    \item \textbf{New}: A task that is defined but is pending development, or a bug that is pending resolution.
    \item \textbf{Active}: A task that is currently in progress. As clarified in Section \ref{sec:methodology}, there are up to 3 tasks in Active status, according to the WIP limit.
...
A communication tool used for conducting regular meetings with the Product Owner, which is mentioned earlier in Section \ref{sec:methodology}.
